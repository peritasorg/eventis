...
-- Add unique constraint to prevent duplicate form templates per event
-- Only for active forms to allow reusing templates after deletion
ALTER TABLE event_forms 
ADD CONSTRAINT unique_active_form_template_per_event 
UNIQUE (event_id, form_template_id, tenant_id) 
