-- Fix security functions and indexes (fixing the immutable function issue)
...
-- 2. Add simpler indexes for security performance (without time predicates)
...
CREATE INDEX IF NOT EXISTS idx_activity_logs_action_tenant 
ON activity_logs(tenant_id, action, created_at) 
WHERE security_event = true;

-- 3. Create automatic security cleanup schedule
